...
//	private Icon getIcon(String pathImage) {
...
//	}
	...
	public inforcustomer() {
		jframecustomer();
		jbuttoncustomer();
		//btadd.setIcon(new ImageIcon("D://HK1//add.png"));
		...
		tbcus.addMouseListener(new MouseListener(){
			...
			public void mouseClicked(MouseEvent e) {
				...
				jcbaddress.setSelectedItem((String)tbcus.getValueAt(row, 4));
				...
				tfemail.setText((String)tbcus.getValueAt(row, 6));
				tftimerent.setText((String)tbcus.getValueAt(row, 7));
				((JTextField)cltimestart.getDateEditor().getUiComponent()).setText(tbcus.getValueAt(row,8 ).toString());
	...
	public void DisplayCustomerfind()  {
		...
		String column[] = {"CMNDcus", "FullName","Birthday","Gender","HomeTown",
	        ...
	        public void jtable() {
	    		...
	    		TitledBorder tbdll= BorderFactory.createTitledBorder(bd2, "Information Customers");
	    				...
	    				String HomeTown = jcbaddress.getSelectedItem().toString();
	    					  try {
	    						  ...
	    						  if(HomeTown != "Home Town") {
	    							  int record = cn.executeDB("Update Inforcustomer set HomeTown = N'"+HomeTown+"'"
	    										+ " where CMNDcus = '"+tfcmnd.getText()+"'");
	    							  if (record>0);
	    							  else JOptionPane.showMessageDialog(null, "Update Home Town was Fail");	
	    						  }
	    						  ...
	    						  if(tffullname.getText().length()!=0) {
	    								  int record = cn.executeDB("Update Inforcustomer set FullName = N'"+tffullname.getText()+"'"
	    											+ " where CMNDcus = '"+tfcmnd.getText()+"'");
	    								  if (record>0)  ;
	    						  ...
	    						  if(tftimerent.getText().length()!=0) {
	    							  int record = cn.executeDB("Update Inforcustomer set TimeRental = '"+tftimerent.getText()+"'"
	    										+ " where CMNDcus = '"+tfcmnd.getText()+"'");
	    							  if (record>0) ;
	    							 else JOptionPane.showMessageDialog(null, "Update time rental was Fail");
	    					  }
	    						  if(((JTextField)clbirthday.getDateEditor().getUiComponent()).getText().length()!=0) {
	    								int record = cn.executeDB("Update Inforcustomer set Birthday = '"+((JTextField)clbirthday.getDateEditor().getUiComponent()).getText()+"' where CMNDcus = '"+tfcmnd.getText()+"'");
	    								if (record>0) ;
	    								else JOptionPane.showMessageDialog(null, "Update Birthday of customer was Fail");
	    							}
	    						  if(((JTextField)cltimestart.getDateEditor().getUiComponent()).getText().length()!=0) {
	    								int record = cn.executeDB("Update Inforcustomer set TimeStart = '"+((JTextField)cltimestart.getDateEditor().getUiComponent()).getText()+"' where CMNDcus = '"+tfcmnd.getText()+"'");
	    								if (record>0) ;
	    								else JOptionPane.showMessageDialog(null, "Update TimeStart was Fail");
	    							}
	    					} catch (Exception e1) {
	    						// TODO Auto-generated catch block
	    						e1.printStackTrace();
	     ...
	     public void checkOrdetemail(String str ) throws Exception {
	 		...
	 		if(b==false) JOptionPane.showMessageDialog(null, "Dia chi Email khong hop le");
