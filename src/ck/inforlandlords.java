...
public class inforlandlords extends JFrame{
	...
	JLabel lbnumberH = new JLabel("Number of House");
	JTextField tfnumberH = new JTextField(15);
	...
	public inforlandlords() {
		...
		tbland.addMouseListener(new MouseListener() {
			public void mouseClicked(MouseEvent e) {
				...
				tfnumberH.setText((String)tbland.getValueAt(row, 6));
			}
	...
	public void jframelandlords() {
		...
		pnnumber.add(lbnumberH);
		pnnumber.add(tfnumberH);
		lbnumberH.setForeground(Color.BLUE);
		pnnumber.setLayout(new GridLayout(1,2));
		...
		pnall.setLayout(new GridLayout(7,1));
		...
		btadd.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				ConnectDBck cn =new ConnectDBck();
				 ...
				 try {
					 checkOrdetemail(tfemailland.getText() );
				 Date selectedDate = (Date) dcbirthdayll.getDate();
					java.sql.Date Birthday = convertUtilToSql(selectedDate);
					int record = cn.executeDB("Insert into inforlandlords values('"+tfCMll.getText()+"','"+tfnamell.getText()+"','"+tfaddressll.getText()+"','"+Birthday+"','"+tfsdt.getText()+"','"+tfemailland.getText()+"',"+tfnumberH.getText()+")");
		...
		btupdate.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					e2.printStackTrace();
				}if (tfnumberH.getText().length()!=0) {
					int record = cn.executeDB("Update inforlandlords set NumberOfHouses = '"+tfnumberH.getText()+"' where CMNDland = '"+tfCMll.getText()+"'");
					if (record>0) ;
					else JOptionPane.showMessageDialog(null, "Update Phone NumberOfHouses was Fail");
				}
				DisplayLandlords();
			}
		});
	...
	// trong sql thì phải dùng DefaultTableModel nếu k sẽ bị trùng dữ liệu
		rs=conn.listAll("Select * from inforlandlords where CMNDland = '"+tfCMll.getText()+"'"
				+ "or FullName = '"+tfnamell.getText()+"' or Address = '"+tfaddressll.getText()+"'"
						+ "or PhoneNumber = '"+tfsdt.getText()+"' or Email = '"+tfemailland.getText()+"' or NumberOfHouse = "+tfnumberH+"");
		String column[] = {"CMND LandLords", "Full Name","Address","Birthday",
			"Phone Number","Email","Number Of House"};
        DefaultTableModel tm = new DefaultTableModel (column, 0);
